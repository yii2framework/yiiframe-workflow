...
abstract class WorkflowBaseObject extends BaseObject
{
	...
	public function __construct($config = [])
	{
		...
		if( array_key_exists('source', $config) ) {
			...
			if( ! $this->_source instanceof IWorkflowSource){
				throw new InvalidConfigException('The "source" property must implement interface yiiframe\workflow\source\IWorkflowSource');
			}
			unset($config['source']);
		}
	 ...
	 * @throws \yiiframe\workflow\base\WorkflowException
	 ...
	 * 
	 * @return \yiiframe\workflow\source\IWorkflowSource the source instance or null if no 
